...
public class AuthorizationController : Controller
{
    ...
    public async Task<IActionResult> Exchange()
    {
        ...
        if (request.IsClientCredentialsGrantType())
        {
            ...
            // Adicionar audience baseado nos scopes
            var scopes = request.GetScopes();
            var audiences = new List<string>();

            if (scopes.Any(s => s.StartsWith("orders.")))
            {
                audiences.Add("orders-api");
            }

            if (scopes.Any(s => s.StartsWith("configurations.")))
            {
                audiences.Add("configurations-api");
            }

            foreach (var audience in audiences)
            {
                identity.AddClaim(OpenIddictConstants.Claims.Audience, audience);
            }
